...
function SWEP:PrimaryAttack()
	...
	if self.StationaryAimingOn and self.Primary.ConeZoomed then
		cone = self.Primary.ConeZoomed
	end
	...
	if self.CustomPrimaryAttack then
	...
	if self.Reloading then
		...
		if not self.CustomReload and (self.LastClip or 0) < self.Weapon:Clip1() then
			...
			if self.LastClip == 0 and self.Weapon:Clip1() == self.Primary.ClipSize and (not self.NoChamberingRounds) then
				...
				if SERVER then
					self.Owner:GiveAmmo(1, self.Weapon:GetPrimaryAmmoType(), true)
				end
			end
		end
	else
		self:SetAiming((self.Owner:KeyDown(IN_ATTACK2) or (bulletTime:GetBool() and bulletTimeOnJump:GetBool() and not util.QuickTrace(self.Owner:GetPos(), Vector(0, 0, -16), self.Owner).Hit)) and ((self.DrawTime or 0) + 0.8 < CurTime()))
	end
...
function SWEP:Deploy()
	...
	if SERVER and game.SinglePlayer() then
		timer.Simple(0.05, function()
			self.Owner.SendLua("if IsValid(LocalPlayer():GetActiveWeapon()) then LocalPlayer():GetActiveWeapon():Deploy() end")
		end)
	end
...
function SWEP:DoShootBullet(dmg, numbul, cone)
	...
	if bulletTime:GetBool() and bulletTimeTracers:GetBool() and ((self.DegreeOfZoom or 0) > 0) then
		tracers = 1
	end
...
local function HUDShouldDraw(name)
	...
	if aimHideHUD:GetBool() and WeaponUsesStationaryAiming then
		prog = LocalPlayer():GetActiveWeapon().DegreeOfZoom
		if prog and prog > 0.5 then return false end
	end
end
...
local function StationaryAimingCalcView(pl, origin, angles, view)
	if WeaponUsesStationaryAiming() then
		...
		if pl:GetActiveWeapon().ViewModelFlip then
