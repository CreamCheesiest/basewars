...
local function DrawDots( w, h, inNest )
	...
	for i, v in pairs( ents.FindInSphere( p_pos, maxRange ) ) do
		if v:IsPlayer() and v != LocalPlayer() or v:GetClass() == 'bw_base_moneyprinter' or v:GetClass() == 'jammer'  then
	...
	for i, v in pairs( targets ) do
		if IsEntity( v ) then
			...
			if( IsJammed and v:GetClass() == 'jammer' ) then 	
				if  nextglitch <= CurTime() then
					nextglitch = CurTime() + math.random( 5, 8 ) / 10
					glitchamount = math.random( 2, 60 )
				end
				surface.SetDrawColor( 0, 0, 0, 255)
					surface.DrawTexturedRect( 
						w * b_c_x - dotSizeX / 2 + rndx + pos.x * w * glitchamount,
