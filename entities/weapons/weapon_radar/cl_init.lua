...
local function DrawDots( w, h, inNest )
	...
	for i, v in pairs( targets ) do
		if IsEntity( v ) then
			...
			if( IsJammed and v:GetClass() == 'bw_base_jammer' ) then 	
				if  nextglitch <= CurTime() then
					nextglitch = CurTime() + math.random( 5, 8 ) / 10
					glitchamount = math.random( 40, 100 )
                    
                    surface.SetDrawColor( 0, 0, 0, 255)
                    surface.DrawTexturedRect( 
                        w * b_c_x - dotSizeX * glitchamount / 2 + rndx * w * 0.80,
                        h * b_c_y - dotSizeY / 2 + rndy * ( h * b_c_y ),
                        dotSizeX, 
                        dotSizeY )
                end
            end 
