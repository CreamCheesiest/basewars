...
local rt = GetRenderTargetEx('___rtaiscannerscreen', 512, 512, RT_SIZE_OFFSCREEN, MATERIAL_RT_DEPTH_NONE, 16, CREATERENDERTARGETFLAGS_UNFILTERABLE_OK, IMAGE_FORMAT_DEFAULT)
...
local function DrawDots(w, h)
	...
	local rndx, rndy = (math.random(-10, 10) / 10) * (maxDotSizeX / 35), (math.random(-10, 10) / 10) * (maxDotSizeY / 35)
...
function DrawMotionTracker()
	...
	if not IsValid(wep) or wep:GetClass() ~= 'weapon_radar' then
		StopNoiseSound()
