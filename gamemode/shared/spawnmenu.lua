...
if SERVER then
	local function Spawn(ply, cat, subcat, item)
		...
		if not ply:Alive() then
			ply:Notify(BaseWars.LANG.DeadBuy, BASEWARS_NOTIFICATION_ERROR)
		...
		if ent then
			...
			if newEnt.SpawnFunction and sf then
				...
				if newEnt.SetUpgradeCost then
					newEnt:SetUpgradeCost(price)
				end
		...
		if not prop then
			prop = ents.Create(ent or "prop_physics")
		end
...
function PANEL:SwitchTo(name, instant)
	...
	if oldpnl then
		oldpnl:AlphaTo(0, instant and 0 or 0.2, 0, function(_, pnl)
			pnl:Hide()
		end)
	end
	...
	return function(pnl)
		...
		for catName, subT in SortedPairs(Models[type]) do
			...
			for name, tab in SortedPairsByMemberValue(subT, "Price") do
				...
				if tab.Gun and (not level or level < BaseWars.Config.LevelSettings.BuyWeapons) then
					level = BaseWars.Config.LevelSettings.BuyWeapons
				end
				...
				function icon:DoClick()
					...
					if (money > 0) and not (myMoney / 100 > money) then
						...
						Derma_Query(string.format(BaseWars.LANG.SpawnMenuBuyConfirm, name, BaseWars.NumberFormat(money)), BaseWars.LANG.SpawnMenuConf, "   " .. BaseWars.LANG.Yes .. "   ", DoIt, "   " .. BaseWars.LANG.No .. "   ")
					else
						DoIt()
					end
				end
