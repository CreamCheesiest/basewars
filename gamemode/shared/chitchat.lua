...
chitchat.MessageModes = {
	{
		PrintName = "GLOBAL",
		HideTag = true,
		Filter = function(s, r) return true end,
	}, -- Global Chat
	{
		PrintName = "TEAM",
		Color = Color(127, 190, 0),
		Filter = function(s, r) return s:Team() == r:Team() end,
	}, -- Team Chat
	{
		PrintName = "LOCAL",
		Color = Color(102, 204, 255),
		Filter = function(s, r) return GetIPR(s) == GetIPR(r) end,
	...
	}, -- Whisper (only people in a 72 unit radius can hear you)
	{
		PrintName = "@ADMIN",
		Color = Color(102, 204, 255),
		Filter = function(s, r) return r:IsAdmin() end,
	}, -- Admins
	{
		-- OOC
		PrintName = "OOC",
	...
	},
	{
		...
		Filter = function(s, r) return s:GetPos():DistToSqr(r:GetPos()) < 112500 end,
	},
}
...
if SERVER then
	...
	function GM:ChangeChatMode(ply, msg, msgmode)
		if msg:StartWith("@") then return 5, msg:gsub("^@", "") end
		if msg:StartWith("//") then return 6, msg:gsub("^//%s?", "") end
		if msg:StartWith("/ooc") then return 6, msg:gsub("^/ooc%s?", "") end
		if msg:StartWith(".//") then return 7, msg:gsub("^%.// ?%s?", "") end
	end
	...
	net.Receive(Tag, function(len, ply)
		...
		if not unmsg then
			Msg("[CHCH] ")
			print(ply, " message failed to decompress!?")
		...
		if not msg then
			Msg("[CHCH] ")
			print(ply, " MESSAGE FAILED TO COMPRESS!? WTF???")
		...
		if net.BytesWritten() > NET_SAFE_LIMIT then
			Msg("[CHCH] ")
			print(ply, " net message len over safe limits, bail")
