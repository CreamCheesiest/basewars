...
function bwa.Error(ply, msg)
	if msg then
		ply:SendLua(string.format("local s = '%s' notification.AddLegacy(s, NOTIFY_ERROR, 5) MsgN(s)", "bwa: " .. msg))
	end
...
-- Commands ---------------------------------------------------
---------------------------------------------------------------
bwa.Reasons = {"Cheating", "Mingebag", "Too young", "Multi-Basing", "Prop-Blocking People", "Exploiting", "Multi-Faction Base",}
	...
	MenuOpen = function(self, option, ent, tr)
		...
		for k, v in next, options do
			submenu:AddOption(v, function()
				self:KickOption(ent, v)
			end)
		end

		submenu:AddOption("Custom...", function()
			Derma_StringRequest("Custom Kick Reason", "Kick " .. ent:Nick() .. " for...", "", function(t)
				self:KickOption(ent, t)
			end)
		end)
	end,
})
