...
-- Woah, no BaseWars table? This should be loaded AFTER the table's created!
if not BW then
...
-- Woah, no UTIL table? This should be loaded AFTER the table's created!
if SERVER and not BW.UTIL then
...
function ModuleLoader:Load()
	...
	for fName in self:IterateFiles("modules") do
		...
		if not ok then
			...
			Log("There was an error loading the module ", colorGreen, "\"", name, "\"", colorWhite, ".", "The error is:\n", err)
			continue
		end
