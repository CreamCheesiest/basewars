...
if SERVER then
	function MODULE:Init(ply)
		...
		if not file.Exists(tag_escaped .. "/" .. dirName .. "/level.txt", "DATA") then
			file.Write(tag_escaped .. "/" .. dirName .. "/level.txt", 1)
		end

		if not file.Exists(tag_escaped .. "/" .. dirName .. "/xp.txt", "DATA") then
			file.Write(tag_escaped .. "/" .. dirName .. "/xp.txt", 0)
		end
