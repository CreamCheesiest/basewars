...
function MODULE:CheckForNULL()
	if not IsFaction and (not BaseWars.Ents:ValidPlayer(Participant1) or not BaseWars.Ents:ValidPlayer(Participant2)) then return false end
...
function MODULE:CheckRaidable(ply, nocool, fac)
	...
	if Call == false then return false, Message end
...
function MODULE:Start(ply, target)
	...
	if Faction then
		Faction.__RaidCoolDown = 0
	else
		ply.__RaidCoolDown = 0
	end
	...
	if Faction2 then
		Faction2.__RaidCoolDown = CurTime() + TimeRemaining
	else
		target.__RaidCoolDown = CurTime() + TimeRemaining
	end
