...
function MODULE:ChangePass(ply, newpass)
	...
	if CLIENT then return end -- Do netmessage here -- check newpass as string
...
function MODULE:InFaction(ply, name, leader)
	...
	if CLIENT then
		...
		if not name then return Fac ~= "" end
	...
	if not name then return Fac ~= "" and Leader end
...
function MODULE:FactionExist(name)
	...
	local Table = BaseWars.Factions.FactionTable
	if Table[name] then return true end
