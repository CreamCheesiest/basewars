...
if SERVER then return end -- FastDL stuff, it gets already automatically added
...
function MODULE:Paint()
	...
	local plTime = me.GetPlayTimeTable and me:GetPlayTimeTable() or {
		h = 0,
		m = 0,
		s = 0
	}
	...
	if triggered then
		...
		local function effectDraw(i)
			...
			change = change / 1.08
			draw.DrawText((fDiff > 0 and "+ " .. BaseWars.LANG.CURRENCY or " " .. BaseWars.LANG.CURRENCY) .. BaseWars.NumberFormat(fDiff), tag, posX, posY, color, TEXT_ALIGN_LEFT)
			posY = posY + change
		end
