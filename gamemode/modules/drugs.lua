...
MODULE.EffectTable = {
	["Stun"] = {
		...
		Check = function(ply)
			...
			if ply.StunDuration < 1 then return DRUG_REMOVE end
		end,
		Remove = function(ply)
	...
	["Poison"] = {
		Apply = function(ply, dur, attk, infl)
			...
			if ply.PoisonDuration > 70 then
				ply.PoisonDuration = 70
			end
		...
		Check = function(ply)
			...
			if ply.PoisonDuration < 1 then return DRUG_REMOVE end
		end,
		Remove = function(ply)
	...
	["Shield"] = {
		...
		Check = function(ply) end,
		-- None. -- Handled in EntityTakeDamage.
		Remove = function(ply)
