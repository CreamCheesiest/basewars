...
function GM:SetupMove(ply, move)
	...
	if not ply:Alive() then return State end
...
function GM:PlayerShouldTakeDamage(ply, atk)
	if aowl and ply.Unrestricted then return false end
	if ply == atk then return true end
...
function GM:Think()
	...
	if LastThink < CurTime() - 5 then
		...
		for k, s in next, Spawns do
			...
			if #Ents < 2 then continue end

			for _, v in next, Ents do
				if v.BeingRemoved or v.NoFizz then continue end
				local Owner = v:CPPIGetOwner()
				if not Owner or not IsValid(Owner) or not Owner:IsPlayer() then continue end
