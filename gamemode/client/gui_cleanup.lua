local blacklist = {"DMenuBar", "DMenu", "SpawnMenu", "ContextMenu", "ControlPanel", "CGMODMouseInput", "Panel",}

local lightblacklist = {"scoreboard", "menu", "f1", "f2", "f3", "f4", "playx", "gcompute",}
...
local function VGUICleanup()
	...
	for _, pnl in next, vgui.GetWorldPanel():GetChildren() do
		...
		Msg("[vgui] ")
		print("Removed " .. tostring(pnl))
		pnl:Remove()
		sum = sum + 1
	end

	Msg("[vgui] ")
	print("Total panels removed: " .. sum)
end
